"""Article variant of fig_component_maps: full-field sigma_xx / sigma_yy / tau_xy PSD maps
at the RESONANT frequency only (truth vs recovered), dropping the off-resonant column that
the letter (fig_component_maps.py / fig_component_maps.pdf) keeps for reviewer R1.11.
...
# resonance = first-mode line (same target as the letter figure)
...
# narrower figure: only one truth/recovered pair per row instead of two
fig = plt.figure(figsize=(95 * MM_TO_INCH, 128 * MM_TO_INCH))
# cols: [truth, rec, cbar]
gs = fig.add_gridspec(3, 3, width_ratios=[1, 1, 0.08],
                      left=0.155, right=0.93, bottom=0.055, top=0.955,
                      wspace=0.08, hspace=0.12)

for r, (lab, c) in enumerate(COMPS):
    tru = np.abs(res['truth'][c][k_res]) / 1e12       # -> MPa^2/Hz
    rec = np.abs(res['recovered'][c][k_res]) / 1e12
    vmax = float(np.nanmax([tru.max(), rec.max()]))
    _, _, Gt = to_grid(tru, x, y)
    _, _, Gr = to_grid(rec, x, y)
    axt = fig.add_subplot(gs[r, 0]); axr = fig.add_subplot(gs[r, 1])
...
save_fig(fig, 'fig_component_maps_resonant')
